...
公開ページはDiscordやnetkeibaを直接取得しません。Vercel Cronまたは保護された管理APIが、取得・照合・保存を先に済ませます。非公開Vercel Blobの週別スナップショットからHTMLを事前生成し、Vercel CDNから配信します（ISR再生成間隔60秒、再生成時もBlobの読取りのみ）。

1. Discordメモを初回だけ履歴取得し、以後はチャンネルごとの最新メッセージID以降だけ増分取得
2. netkeibaの週末一覧を1回だけ取得し、日付別に必要な出走表だけを更新
3. 出走馬と保存済みメモを照合
...
普段はVercelの対象プロジェクト → Settings → Cron Jobs → 木曜のジョブ（`0 7 * * 4`）の **Run** から手動更新できます。Discordだけなら月曜のジョブ（`0 20 * * 0`）を実行します。更新中に連打しないでください。公開画面への反映にはISR/CDNの更新で1〜2分程度かかる場合があります。

`all`の実行時、先週の保存データが存在しなければ一度だけ補完します。保存後は先週の出走表を再取得せず、次週以降は通常更新で作った週別データをそのまま再利用します。取得失敗・未公開のレースは前回分を保持し、診断欄に警告を出します。

Discordは最新100件のページから保存済みIDまで戻る方式です。通常は各チャンネル1回、新着が100件を超える場合だけ必要ページを追加取得します。既存ID以下の本文は再登録・再照合対象への追加をしません。過去投稿の編集・削除は自動追跡しません。新着が設定上限を超えた場合は欠落を防ぐためカーソルを進めず警告します。
...
既存のGitHub `main` → Vercel `wins-jinro-keiba-memo-vercel-v3` の自動デプロイを使用します。新規ProjectやURLは作成しません。Productionに設定済みのDiscord環境変数は変更不要です。
...
### DISCORD_CHANNEL_IDS
設定済みの2チャンネルをそのまま使用します。値はリポジトリに記載しません。
...
これらは既存設定をそのまま使用します。履歴範囲は初回バッチの取得時間に影響しますが、ページ表示時の外部取得は発生しません。
...
通常のURLは今週、`?week=last` は先週の保存済みスナップショットを表示します。いずれも閲覧時のDiscord・netkeiba取得はありません。日本時間の月曜は3日間開催の最終日として土日月を維持し、火曜から次週末に切り替わります。実際にレースが取得できた日だけ開催タブに表示します。
